extern int money;         // 钱数 （分数）
extern int boat_capacity; // 船装货上限
// 到对应泊位的距离
extern int berth_dis[n][n][berth_num];
// 到随机点的距离
extern int random_dis[n][n][random_bfs_point + 50];
// 所有随机点的坐标
extern vector<pair<int, int>> random_point;
#ifdef _WIN32
...
void Init()
{
    ...
    {
        if (random_point.size() >= random_bfs_point)
            break;
        for (int jj = 0; jj < n; jj++)
        {
            if (random_point.size() >= random_bfs_point)
                break;
            if (game_map[ii][jj] == '#' || game_map[ii][jj] == '*' || vis[ii][jj])
                continue;
    ...
    while (have_robot && !new_cargos.empty())
    {
        ...
        for (int i = 0; i < robot_num; i++)
        {
            ...
            if (robots[i].goods == 1)
                continue;
            // 如果机器人到不了对应的泊位
            if (berth_dis[robots[i].x][robots[i].y][berth_id] == -1)
                continue;
            int robot_time = CargotoRobot(cargo, robots[i]);
            if (robot_time == MAXNUM)
                continue;
